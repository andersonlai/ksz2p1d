import numpy as np
import healpy as hp
from scipy.special import sph_harm_y, sph_harm_y_all, legendre_p, legendre_p_all
...
def sph_harm_y_real_all(lmax, theta, phi):
    ...
    phi = np.asarray(phi)
    shape = theta.shape
    out = np.zeros((lmax+1, 2*lmax+1) + shape, dtype=np.float64)
...
def get_Pl_ij_my(mask,lmax=None):
...
# take a mask instead of nside
def theta_phi_my(mask):
    npix = mask.shape[0]
    nside = hp.npix2nside(npix)
    pix_idxs = np.arange(npix)[mask.astype('bool')]
    theta, phi = hp.pix2ang(nside, pix_idxs)
    return theta, phi
